/** Seções visuais do relatório de tráfego (Meta, Google, Landing Page) —
 *  usadas pelo relatório mensal salvo (MonthlyReportPage) e pelo link
 *  público com datas livres (PublicReportPage). */
import { useMemo, useState, type ReactNode } from 'react'
import { format, eachDayOfInterval } from 'date-fns'
import { ptBR } from 'date-fns/locale'
import { Wallet, Eye, Users, MousePointerClick, Percent, Coins, Gauge, MessageCircle, DollarSign, Trophy } from 'lucide-react'
import { ReportLineChart, type ChartSeries } from './ReportLineChart'
import { buildExecutiveSummary, buildFunnelSentence, buildGoogleExecutiveSummary, pctChange } from '../../utils/reportSummary'
 ...
 *  checagem em cada um). */
export type ReportGoogleSnapshotReady = Extract<ReportGoogleSnapshot, { available: true }>

/* ---------- formatters ---------- */
export const fmtInt = (v?: number, masked?: boolean) => (masked ? '•.•••' : v == null || Number.isNaN(v) ? '—' : Math.round(v).toLocaleString('pt-BR'))
export const fmtBRL = (v?: number, masked?: boolean) =>
  masked ? 'R$ •.•••,••' : v == null || Number.isNaN(v) ? '—' : v.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })
export const fmtPct = (v?: number, masked?: boolean) => (masked ? '•,••%' : v == null || Number.isNaN(v) ? '—' : `${v.toFixed(2).replace('.', ',')}%`)
export const fmtDate = (d: Date) => (Number.isNaN(d?.getTime?.()) ? '—' : format(d, 'dd/MM/yyyy', { locale: ptBR }))
...
export function Delta({ curr, prev, goodWhen = 'up' }: { curr?: number; prev?: number; goodWhen?: 'up' | 'down' }) {
...
export function OverviewSection({ meta }: { meta: ReportMetaSnapshot }) {
...
export function FunnelSection({ meta }: { meta: ReportMetaSnapshot }) {
...
export function EvolutionSection({ meta, periodStart, periodEnd }: { meta: ReportMetaSnapshot; periodStart: Date; periodEnd: Date }) {
  const { isPrivacyMode } = usePrivacy()
...
export function CampaignsSection({ campaigns }: { campaigns: ReportEntitySummary[] }) {
  const { isPrivacyMode: m } = usePrivacy()
...
/* ---------- Section 5: top ads ---------- */
export function AdsSection({ ads }: { ads: ReportEntitySummary[] }) {
  const { isPrivacyMode: m } = usePrivacy()
  const ranked = [...ads]
...
export function PlatformSection({ meta }: { meta: ReportMetaSnapshot }) {
...
export function GoogleOverviewSection({ google }: { google: ReportGoogleSnapshotReady }) {
  const { isPrivacyMode: m } = usePrivacy()
...
export function GoogleEvolutionSection({
  google,
  periodStart,
  periodEnd,
}: {
...
export function GoogleCampaignsSection({ campaigns }: { campaigns: ReportGoogleCampaignSummary[] }) {
  const { isPrivacyMode: m } = usePrivacy()
...
export function LandingPageSection({ lp }: { lp: ReportLandingPageSnapshot }) {
...
/* ---------- Resumo ---------- */
/** Resumo em texto corrido (Meta e/ou Google) + os próximos passos, quando houver. */
export function ExecutiveSummary({
  ...
  nextSteps?: string
  /** Espaço pro editor de próximos passos (só no relatório interno). */
  children?: ReactNode
}) {
  const text = [meta ? buildExecutiveSummary(meta, start, end) : null, google ? buildGoogleExecutiveSummary(google, start, end) : null]
    .filter(Boolean)
    .join('\n\n')
  return (
    <div className="rounded-2xl bg-[#F8FAFC] p-6">
      {text && <p className="whitespace-pre-line text-[15px] leading-relaxed text-slate-700">{text}</p>}
      {children ??
        (nextSteps ? (
          <div className="mt-5 border-t border-[#E2E8F0] pt-4">
            <p className="mb-1 text-sm font-semibold text-slate-800">Próximos passos</p>
            <p className="whitespace-pre-line text-[15px] leading-relaxed text-slate-700">{nextSteps}</p>
          </div>
        ) : null)}
    </div>
  )
