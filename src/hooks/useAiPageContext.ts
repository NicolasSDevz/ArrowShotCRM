...
export function useAiPageContext(): { context: AiPageContext; resolveContext: () => Promise<AiPageContext> } {
      ...
      if (client) {
        ...
        return {
          ...
          suggestedQuestions: [
            ...
            '🔎 Quais termos de pesquisa negativar?',
  ...
  const resolveContext = useCallback(async (): Promise<AiPageContext> => {
    ...
    // Palavras-chave, termos de pesquisa e níveis de campanha/conjunto/anúncio
    // o próprio Archer busca no servidor quando a pergunta pede (ferramentas
    // em api/ai/chat.js), de qualquer cliente — não precisa pré-carregar aqui.
    return {
      ...context,
